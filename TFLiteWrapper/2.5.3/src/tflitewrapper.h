 ...
 * The function does not allocate memory, it just passes the pointer to the first element of the array with the .data()
 * method.
 *
 * @tparam IN_SIZE
 * @tparam OUT_SIZE
...
int classifyFlat2D(ClassifierPtr cls, std::array<float, N_ROWS * N_COLS> &flatInputMatrix, std::array<float, OUT_SIZE> &outputArray) {
    return classifyFlat2D(cls, flatInputMatrix.data(), N_ROWS, N_COLS, outputArray.data(), (size_t)OUT_SIZE);
}
